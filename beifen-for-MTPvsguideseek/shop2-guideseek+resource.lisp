...
(defun seek-plans (state tasks top-tasks partial-plan partial-plan-cost
  ...
  (format t "run into seek plan stop-tasks=~A~%" top-tasks)
  (when (time-expired-p)
      ...
      (progn
        (if (string-equal "atom" task-name :start1 0 :end1 4 :start2 0 :end2 4)
            (progn 
              ...
              (guide-seek-plans task1 task-name task-body state tasks top-tasks
...
(defun seek-plans-primitive (task1 task-name task-body state tasks top-tasks
  ...
  (format t "run into seek-plans-primitive ~%")
   ...
   (when (eql result1 'fail)
    ...
    (let ((new-cost (+ cost partial-plan-cost)))
     ...
     (seek-plans state tasks1 top-tasks1 
      ...
      (format t "retract-from task1=~A~%" task1)
     (retract-state-changes state tag)
     nil)))
