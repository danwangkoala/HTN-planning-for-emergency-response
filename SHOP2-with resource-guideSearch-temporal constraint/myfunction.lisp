...
(defun guide-seek-plans (task task-name task-body state tasks top-tasks temporal-network
  ...
  (let* (task-method task-start task-end check-tasks temporal-network1) 
          ...
          (if (EQL x1 'consistent)
              ...
              (progn 
                ...
                (if (EQL x2 'consistent)
                    ...
                    (return-from guide-seek-plans nil)))))
        (progn
          (setq temporal-network1 (real-guide-seek-plans task task-name task-body task-start task-end state tasks top-tasks temporal-network 
...
(defun real-guide-seek-plans (task task-name task-body start end state tasks top-tasks temporal-network check-tasks partial-plan partial-plan-cost
  ...
  (if (eql 1 (length check-tasks))
      (progn 
        (seek-plans-nonprimitive-atom task task-name task-body state tasks top-tasks temporal-network
                               partial-plan partial-plan-cost depth
                               which-plans protections)
  ...
  (dolist (task1 check-tasks)
    ...
    ;(format t "real-guide-seek-plans dolist task1=~A~%" task1)
      (let* ((task1-name (get-task-name task1))
             (task1-body (get-task-body task1))
             task1-start task1-end task1-method temporal-network1)
      (if (primitivep task1-name)
          (progn 
           (seek-plans-primitive task1 task1-name task1-body state tasks top-tasks temporal-network
                            partial-plan partial-plan-cost depth which-plans
                  ...
                  (let* ((task1-end0 (third task1-body)) end-constrain end-constrain1)
                    ...
                    (when (or (null end-constrain) (< (car (second end-constrain)) 0))
                      ...
                      (if (< task1-end end)   ;;;various conditions, depending on search mechanism(other search mechanism can be added here)
                          (progn
                            (setq temporal-network1 (real-guide-seek-plans task1 task1-name task1-body task1-start task1-end state tasks top-tasks temporal-network check-tasks
            ...
            (let* (starttime endtime)
              (dolist (x (car state-unifiers))
                ...
                (if (string-equal "end" (car x) :start1 0 :end1 3 :start2 1 :end2 4)
                    (progn
                      (setf endtime (cdr x)))))
              (return-from obtain-action-time (values starttime endtime temporal-network)))  
    ...
    (if (null temporal-network)
        (progn
          (let* (x)
            ...
            (setq temporal-network (list (list x) (list (second e) (third e))))))
        (progn
          (let* (x y)
            ...
            (setq temporal-network (list x y))))))
  temporal-network)
...
(defun seek-plans-nonprimitive-atom (task1 task-name task-body state tasks 
                                      ...
                                      depth which-plans protections)
  (format t "run into seek plans nonprimitive task=~A~%" task1)
  (setq *temporal-network* temporal-network)
