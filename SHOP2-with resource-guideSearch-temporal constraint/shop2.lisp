  ...
  (let ((standardized-method (standardize method))  
    ...
    (unless (eql task-unifier 'fail)
      ...
      (do* ((body (cddr standardized-method) (cdddr body)))
        ...
        (if state-unifiers
            (let* (temporal-constrain vertexes)
              ...
              (dolist (x (car state-unifiers))
                (if (string-equal "start" (car x) :start1 0 :end1 5 :start2 1 :end2 6)
                    (progn
                      (setq temporal-constrain (adjoin (list (cons 'RT (car x)) (cons (cdr x) (cdr x)) 1) temporal-constrain))
                      (setq vertexes (adjoin (car x) vertexes))))
                (if (string-equal "end" (car x) :start1 0 :end1 3 :start2 1 :end2 4)
                    (progn
                      (setq temporal-constrain (adjoin (list (cons 'RT (car x)) (cons (cdr x) (cdr x)) 1) temporal-constrain))
                      (setq vertexes (adjoin (car x) vertexes)))))
              (setq *temporal-network* (list temporal-constrain vertexes))))   
...
(defun seek-plans-task (task1 state tasks top-tasks temporal-network partial-plan
  ...
  ;(format t "run into seek-plans-task temporal-network=~A~%" temporal-network)
...
(defun seek-plans-primitive (task1 task-name task-body state tasks top-tasks temporal-network
  ...
  (format t "run into seek-plans-primitive task1=~A~%" task1)
  ...
  (let* ((*start-run-time* (get-internal-run-time))
    ...
    (ecase which           
           ((:first :all :shallowest :all-shallowest)
            (catch-internal-time
             (seek-plans state tasks top-tasks nil nil 0 0 which nil))
