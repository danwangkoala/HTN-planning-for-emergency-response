          ...
          (when preconditions
            ...
            (setf start
                 (dolist (x (car pu))
                   ;(print (car x))
                   ;(format t "~a" (car x))
                   ;; ////please use "string-equal" rather than "string="; (car x) follow the form output by (format) rather than print
...
;(format t "~%Sat to explain: ~s" (apply-substitution preconditions unifier))
	  ...
	  (if *explanation*
          ...
          ;;(format t "apply-operator state1= ~A~%" state)
          ...
          ;;(format t "aplly-operator state2= ~A~%" state)
